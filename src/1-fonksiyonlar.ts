...
/*  firstName parametresi zorunlu olarak tanımlanmış durumda ve fonksiyonu çağırırken bu parametreyi sağlamadığınızda TypeScript hata verecektir.
    firstName parametresini opsiyonel yapmak için ? kullanmanız gerekiyor. */
 function kardesPayi(firstName?: string): string{

    if(firstName){
         return `bi tane  ${firstName}  için, bir de benim için`;
    }else
